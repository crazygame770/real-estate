...
interface AveragePropertyPriceGridProps {
...
export const AveragePropertyPriceGrid = ({ 
  neighborhoodPrices, 
  regionAvg, 
  athensAvg, 
  color 
}: AveragePropertyPriceGridProps) => {
  const { t } = useLanguage();
  ...
  const allPrices = neighborhoodPrices.map(item => item.avgPrice);
  
  return (
    <Card className="p-6 mb-8 bg-[#0C1021] text-white border-none">
      <h3 className="text-lg font-semibold mb-2 text-white">{t("Average Property Price")}</h3>
      <p className="text-sm text-gray-400 mb-4">{t("Average price in each neighborhood")}</p>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {/* Map through neighborhoods */}
        {sortedNeighborhoods.map((item) => {
          const intensity = getColorIntensity(item.avgPrice, allPrices);
          ...
          return (
            ...
            >
              ...
              <div className="text-xl font-semibold">€{Math.round(item.avgPrice).toLocaleString()}</div>
            </div>
          );
        })}
        ...
        >
          ...
          <div className="text-xl font-semibold">€{Math.round(regionAvg.price).toLocaleString()}</div>
        </div>
        ...
        >
          ...
          <div className="text-xl font-semibold">€{Math.round(athensAvg.price).toLocaleString()}</div>
        </div>
      </div>
